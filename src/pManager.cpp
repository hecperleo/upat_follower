...
int main(int _argc, char** _argv) {
  ...
  while(ros::ok()){
    ...
    if(path.poses.size() != 0 && flagSpline == true){
			...
			while(flagFinishSpline == false){ // Comentar para tener el generador V1
...
void  eclSpline(float minT, float dist_total){
	if(lastOne == false){ // Comentar para tener el generador V1
...
std::vector<double> interpolaWps(std::vector<double> wp, double t){
	...
	double porcion;
	double newTsize = t;
	if (newTsize<wp.size()){ newTsize = wp.size();}
