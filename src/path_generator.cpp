...
bool PathGenerator::trajectoryCallback(uav_path_manager::GenerateTrajectory::Request &_req_trajectory,
    ...
    for (int i = 0; i < _req_trajectory.time_intervals.data.size(); i++) {
        time_intervals.push_back(_req_trajectory.time_intervals.data.at(i));
    }
...
nav_msgs::Path PathGenerator::pathManagement(std::vector<double> list_pose_x, std::vector<double> list_pose_y, std::vector<double> list_pose_z) {
    ...
    switch (mode_) {
        case mode_interp1_:
            return createPathInterp1(list_pose_x, list_pose_y, list_pose_z, list_pose_x.size(), interp1_final_size);
        case mode_cubic_spline_loyal_:
            return createPathCubicSpline(list_pose_x, list_pose_y, list_pose_z, list_pose_x.size());
        case mode_cubic_spline_:
            return createPathCubicSpline(list_pose_x, list_pose_y, list_pose_z, list_pose_x.size());
