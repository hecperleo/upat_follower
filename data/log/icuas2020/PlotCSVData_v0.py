...
case_name = 'simlight_optitrack_la05_2height'
...
try:
    default_times = pd.read_csv(
        dir_config + 'optitrack2_t.csv', names=['Times'])
except FileNotFoundError:
    print('init.csv not found!')
try:
    default_init_path = pd.read_csv(
        dir_config + 'optitrack2.csv', names=['x', 'y', 'z'])
...
def plot3DFigure(_compare_path, _current_trajectory, _num):
    figN = plt.figure(num='Mode ' + str(_num) + ' 3D behaviour', figsize=(10,8))
    axN = Axes3D(figN)
